# MacroPhase

> **Open beta** — source code released under AGPL at a future date.

An adherence-neutral nutrition and weight tracking app. Log what you eat, track your weight, and let the math figure out the rest.

### Features

- **Food logging** — barcode scanner, AI camera scan, manual search (OpenFoodFacts + CIQUAL)
- **Weight tracking** — trend weight via EWMA, scale weight logging, Health Connect sync
- **Dynamic TDEE** — 14-day rolling window, non-linear tissue density, adaptive confidence scoring
- **AI Coach** — multi-turn conversation with function calling (Gemini AI Studio API, key required, grab for free @ https://aistudio.google.com)
- **Custom foods, recipes, serving presets**
- **Offline-first** — all data in local Room database
- **No account, no cloud, no subscription**

### Beta

This is an early beta. Things will break, change, and improve. Feedback and bug reports are welcome via [Discord](https://discord.gg/qDCXcrFGwz).

### Download

[Latest APK](https://macrophase.github.io/)

### License

MacroPhase is **AGPL-licensed** software. The source code will be published to this repository once the beta period concludes.
